...
*/
        ...
        throws IOException, GeneralSecurityException 
    {
        ...
        if (input != null) {
            ...
            main.putValue("Manifest-Version", "1.0");
            main.putValue("Created-By", "1.0 (无名指)");
        }
 ...
 /*   private static void writeSignatureFile(Manifest manifest, OutputStream out)throws IOException, GeneralSecurityException{
    ...
    */
    public   void loadKeys( String name,Context a)
        throws IOException, GeneralSecurityException
    {
        

        // load the private key
    // privateKey=readPrivateKey(a.getAssets().open("/keys/"+name+".pk8"));
        privateKey=readPrivateKey(a.getAssets().open("keys/"+name+".pk8"));

        // load the certificate
        publicKey=readPublicKey(a.getAssets().open("keys/"+name+".x509.pem"));

        // load the signature block template

        sigBlockTemp=readBytes(a.getAssets().open("keys/"+name+".sbt"));
    }
...
/*    private static void copyFiles(Manifest manifest, JarFile in, JarOutputStream out)throws IOException{
    ...
    */
    ...
    public static void sign(Context a, File in,String ou,boolean b){
      try
       {
        res.loadKeys ( "Legu", a );
        ZipInput input=null;
